...
This program must be called from the top-level (root) directory of a
repository (or, in bzr, a branch checkout). Exception: it can be
called from any level of a Subversion checkout directory, but not from
within a Subversion repository directory.
